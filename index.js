...
let genreList = [] // Get a list of all genres
...
app.get('/', (req, res) => {
    axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=${process.env.API_KEY}`)
        .then(function (response) {
            res.render('pages/index', { genreList, movies: response.data, activePage: 1, genreChosen: false, searchTermEntered: false, searchTerm: '' })
        })
        .catch(function (error) {
            console.log(error);
        })
...
app.get('/page/:page', (req, res) => {
    axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=${process.env.API_KEY}&page=${req.params.page}`)
        .then(function (response) {
            res.render('pages/index', { genreList, movies: response.data, activePage: Number(req.params.page), genreChosen: false, searchTermEntered: false, searchTerm: '' })
        })
        .catch(function (error) {
            console.log(error);
        })
...
app.get('/genre/:id/page/:page', (req, res) => {
    axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${process.env.API_KEY}&with_genres=${req.params.id}&page=${req.params.page}`)
        .then(function (response) {
            res.render('pages/index', { genreList, movies: response.data, activePage: Number(req.params.page), genreChosen: true, genreID: Number(req.params.id), searchTermEntered: false, searchTerm: '' })
        })
        .catch(function (error) {
            console.log(error);
        })
...
app.get('/search/:term/page/:page', (req, res) => {
    axios.get(`https://api.themoviedb.org/3/search/movie?api_key=${process.env.API_KEY}&query=${req.params.term}&page=${req.params.page}`)
        .then(function (response) {
            res.render('pages/index', { genreList, movies: response.data, activePage: Number(req.params.page), genreChosen: false, searchTermEntered: true, searchTerm: req.params.term })
        })
        .catch(function (error) {
            console.log(error);
        })
